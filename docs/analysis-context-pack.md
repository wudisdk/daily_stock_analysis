...
| `factor_snapshot` | Low-sensitivity as-of factor summary: technical score band, price heat, volume-price activity, valuation/quality coverage, fund-flow guard, de-risk guard, risk and confidence labels. | Derived only from already-fetched artifacts; no new fetcher, raw payload dump, or future-return label. |
...
- `factor_snapshot` 只读已传入的 `realtime_quote`、`trend_result` 和 `fundamental_context`，把 DuckDB 风格的 as-of 维度压缩为低敏 label/status：technical score band、price heat、volume-price activity、valuation/quality coverage、fund-flow guard、de-risk guard、risk 和 confidence；不写入原始资金流金额、风险文本或未来收益标签。
